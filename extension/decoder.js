 ...
 * Why it exists
 * -------------
 * The first version used `wimage_wasm` -- the decoder written by Hugi-R for
 * wplace.eralyon.net. Its repository declares no license: embedding that file
 * in a published extension would have meant redistributing his code without the
 * right to do so, and the Chrome Web Store forbids loading it remotely.
 *
 * The format turned out to be simple, so we read it ourselves. Not one line of
 * his code is used: only `fzstd` (MIT) and the wplace palette.
 *
